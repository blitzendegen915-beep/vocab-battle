...
description: 写真スライドショー・ビート同期MV・タイトルモーション等の動画制作ワークフロー。ffmpegベースで素材収集→選定→クリップ生成→結合→音声→納品まで。ユーザーが「動画作って」「スライドショー」「MV風に」「曲に合わせて」と言ったら必ずこのスキルを読む。
---

# 動画制作スキル (ffmpeg スライドショー / ビート同期)

クラウド環境(AviUtl不可)での動画制作の確立済みワークフロー。全工程ffmpeg+Python。
...
```bash
# i%2で交互にズームイン/アウト。DUR秒 → d=DUR*30フレーム
Z_IN="zoompan=z='min(zoom+0.0009,1.16)':d=90:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s=1920x1080:fps=30"
Z_OUT="zoompan=z='if(lte(zoom,1.0),1.16,max(zoom-0.0009,1.0))':d=90:x=...同上"
...
### A. クロスフェード版 (エモ・ポップ用)
xfadeチェーン。offsetは累積時間から算出(Pythonで生成すること。手書き禁止):
```
offset_i = (これまでの合計時間) - XFADE_DUR
cum += dur_i - XFADE_DUR
```
...
- **軽量版を必ず作る**: 161MBはダウンロード失敗した実績あり。`-vf scale=1280:720 -crf 26` で20-30MBに。
- SendUserFileで送付。フルHD版は保管しておき「必要なら」と伝える。
- 完成フレームを2-3枚抽出して自分で目視確認してから送る(`-ss T -frames:v 1`)。

## 9. 進行管理・トークン節約

- 長時間レンダは background Bash + タスク通知待ち。ポーリングするなら `ls clips/*.mp4 | wc -l` 一発で。
- ffmpegログは `| tail -1` に絞る。プレビュー確認は本当に必要な箇所だけ。
- ユーザー確認(AskUserQuestion)は最初に一括: 素材の受け渡し方法 / 雰囲気 / 画面比率 / テンポ。
- 著作権楽曲は使えない旨を伝え、私的利用のユーザー提供音源はmux可。
